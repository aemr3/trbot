...
import { ChatGptAccountService, type ChatGptAccount } from "@trbot/ai/chatgpt-account.ts"
import { createChatGptModel } from "@trbot/ai/chatgpt-provider.ts"
import { ModelOverviewGenerator, type OverviewGenerator } from "@trbot/ai/overview.ts"
import {
  ApiHttpError,
  createApiClient,
...
import { loadConfig, type AppConfig, type AppCredentials } from "@trbot/config"
...
import type { PriceAlertStore } from "@trbot/market/alert.ts"
...
import { ApiBrokerageDistributionSource } from "@trbot/market/api-brokerage.ts"
import { ApiCandleSource } from "@trbot/market/api-candles.ts"
import { ApiDepthStream } from "@trbot/market/api-depth-stream.ts"
import { ApiNewsSource } from "@trbot/market/api-news.ts"
import { ApiSettlementSource } from "@trbot/market/api-settlement.ts"
import { ApiEquityQuoteStream } from "@trbot/market/equity-quote-stream.ts"
import { ApiQuoteStream } from "@trbot/market/quote-stream.ts"
import { ApiViopInstrumentSource } from "@trbot/market/api-source.ts"
import { ApiMemberFeatureSource } from "@trbot/member/api-features.ts"
import type { OverviewSnapshotStore } from "@trbot/market/overview.ts"
import type { StopRuleStore } from "@trbot/trading/stop.ts"
...
import { ApiAccountSource } from "@trbot/trading/api-account.ts"
import { ApiAccountStream } from "@trbot/trading/api-account-stream.ts"
import { ApiViopOrderSource } from "@trbot/trading/api-order.ts"
...
async function resolveInitialState(config: AppConfig): Promise<InitialState> {
  const api = await resumeApiClient(() => openAuthSession(config.databaseUrl), config.credentials)
...
export class App {
  ...
  constructor(
    ...
    this.openAuthSession = options.openAuthSession ?? (() => openAuthSession(config.databaseUrl))
    this.recoverSession =
      options.recoverSession ?? ((credentials) => authenticateApiClient(this.openAuthSession, credentials))
    ...
    this.screen = this.api
      ...
      : new LoginScreen(renderer, this.openAuthSession, {
  ...
  private transitionToLogin(initialUsername?: string): void {
    ...
    this.workspace = null
    this.replaceScreen(
      new LoginScreen(this.renderer, this.openAuthSession, {
...
async function authenticateApiClient(
  ...
  const api = await createApiClient(openAuthSession, credentials)
  try {
