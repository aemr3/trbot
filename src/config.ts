...
export function loadAiModel(env: Record<string, string | undefined> = process.env): string {
  return env.TRBOT_AI_MODEL?.trim() || DEFAULT_AI_MODEL
}

export function loadAiReasoningEffort(env: Record<string, string | undefined> = process.env): string {
