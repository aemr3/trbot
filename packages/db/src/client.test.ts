...
test("rejects a relative path so the working directory cannot decide which database is opened", async () => {
  expect(openDatabase("./data/db.sqlite")).rejects.toThrow(/must be absolute/)
  expect(openDatabase("file:data/db.sqlite")).rejects.toThrow(/must be absolute/)
})

test("accepts an in-memory database", async () => {
