...
describe("ChatGPT account", () => {
  test("persists browser login and removes it on disconnect", async () => {
    ...
    expect(await account.connect()).toEqual({
      providerId: "openai",
      accessToken: "access-1",
      refreshToken: "refresh-1",
      expiresAt: 601_000,
...
function oauth(tokens: Awaited<ReturnType<ChatGptOAuth["login"]>>): ChatGptOAuth {
  return {
    async login() {
      return tokens
    },
